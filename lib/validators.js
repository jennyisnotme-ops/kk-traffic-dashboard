...
// payload 形狀：{ webhook_url, reports: [{report_id, enabled}, ...] }
// webhook 全域共用一組；reports 是「每份自訂報表的勾選狀態」清單，可勾選多份各自獨立啟用
function validateDigestSettings(p) {
  ...
  if (!Array.isArray(p.reports)) return { ok: false, error: 'reports 需為陣列' };
  for (const r of p.reports) {
    if (!r || typeof r !== 'object' || !Number.isInteger(r.report_id) || typeof r.enabled !== 'boolean')
      return { ok: false, error: 'reports 內每筆需含整數 report_id 與布林 enabled' };
  }
  if (p.reports.some(r => r.enabled) &&
      (typeof p.webhook_url !== 'string' || !DISCORD_WEBHOOK_RE.test(p.webhook_url)))
    return { ok: false, error: '有勾選啟用的報表時，webhook 網址為必填且需合法' };
  return { ok: true };
}

module.exports = { validateExportPayload, validateReportConfig, REPORT_FIELDS, ALL_PAGES,
  validateNewUser, validateLayoutCards, validateThemeColor, validateDigestSettings, DISCORD_WEBHOOK_RE };
